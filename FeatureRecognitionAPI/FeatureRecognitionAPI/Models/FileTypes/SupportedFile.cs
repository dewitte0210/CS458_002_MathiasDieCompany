...
public abstract class SupportedFile
{
    ...
    protected SupportedFile(List<Entity> entityList)
    {
            ...
            for (int j = i + 1; j < EntityList.Count; j++)
            {
                ...
                else // both i and j are already mapped
                {
                    ...
                    for (int k = 0; k < listMap.Count; k++)
                    {
                        ...
                        else if (listMap[k] > listMap[j]) listMap[k]--;
                    }
                    listMap[j] = listMap[i];
                }
            }
    ...
    public void SetFeatureGroups()
    {
        ...
        List<Feature> features = new(FeatureList);
    ...
    private static Entity? CadObjectToInternalEntity(ACadSharp.Entities.Entity cadEntity)
    {
        switch (cadEntity)
        {
    ...
    private static FileVersion GetFileVersion(string version)
    {
        switch (version)
        {
            case "AC1006":
    ...
    {
        FileVersion = GetFileVersion(Doc.Header.VersionString);
        ...
        foreach (ACadSharp.Entities.Entity entity in Doc.Entities)
        {
            if (entity is Spline)
            {
                throw new NotImplementedException(
                    "Splines are not yet supported. Please explode your splines before uploading your file.");
            }

            if (entity is LwPolyline)
            {
                throw new NotImplementedException(
                    "Poly lines are not yet supported. Please explode your poly lines before uploading your file.");
            }
    ...
    public void CornerNotchFlagHelper(Line entity)
    {
        ...
        {
            if (entity.AdjList[i] is Line { AdjList.Count: 2 } e && DEQ(entity.GetLength(), e.GetLength()))
            {
